...
2. Set the `CSK_NAME` and `CSK_TYPE` repo variables (Settings → Variables → Actions) — `CSK_TYPE` is `language`, `CSK_NAME` is your language's name (e.g., `french`).
3. Add your translations.
4. Test your language thoroughly.
5. Tag a stable release (`x.y.z`) so your `release.yml` workflow publishes a GitHub Release with the ZIP asset.
6. Contact maintainers for review, with a link to your repository.
7. Once approved, your repository is added to the [official package allowlist](https://github.com/ciskeleton/packages).

> ⚙️ From that point on, CSK automation checks your repository periodically and picks up every new stable release automatically.
